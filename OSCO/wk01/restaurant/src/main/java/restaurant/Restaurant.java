...
public class Restaurant {
  ...
  private final Semaphore orderQueueLock = new Semaphore(1);
  /**
   * Queue of meals.
   */
  private Queue<Meal> mealsReadyQueue;
  private final Semaphore mealsReadyQueueLock = new Semaphore(1);
  /**
...
//                                price ) );
  ...
  public boolean hasOrders() {
    ...
    boolean result = !orderQueue.empty();
    orderQueueLock.release();
    return result;
  }
  ...
  public String getNextOrder() {
    ...
    String result = orderQueue.get().toString();
    orderQueueLock.release();
    return result;
  }
  ...
  public void procesOrders() {
    while (true) {
      ...
      boolean b = this.orderQueue.empty();
      orderQueueLock.release();
      if (b) {
        return;
      }
      ...
      for (OrderLine ol : order) { //while (order.hasOrderLines()) {
        ...
        for (int p = 0; p < persons; p++) {
          try {
            this.mealsReadyQueueLock.acquire();
          } catch (InterruptedException ex) {
            Logger.getLogger(Restaurant.class.getName()).log(Level.SEVERE, null, ex);
          }
          this.mealsReadyQueue.put(this.
                  prepareMeal(orderNR, mealNR));
          this.mealsReadyQueueLock.release();
        }
      }
  ...
  public boolean hasMeals() {
    ...
    boolean result = !mealsReadyQueue.empty();
    this.mealsReadyQueueLock.release();
    return result;
  }
